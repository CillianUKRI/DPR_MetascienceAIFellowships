...
rawcsv <- read_csv("inputdata/export_exptbl_17_07_2024.csv") |>
  select("Reference", "ID", "Applicant_x0020_Name", "Applicant_x0020_Email", "Applicant_x0020_Organisation", "Co_x002d_Investigators") |>
  setNames(c("Reference", "ID", "ApplicantName", "ApplicantEmail", "ApplicantOrganisation", "Co-Is")) |>
  mutate(`Co-Is` = sapply(`Co-Is`, extract_names)) |>
  distinct(ApplicantName, .keep_all = TRUE) # Check there aren't genuine cases of people with the same name
